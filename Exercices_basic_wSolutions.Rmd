...
```{r}
setwd("seminar-youtube-discourses")

getwd()

party_videos <- readRDS("data/party_videos.RDS")

party_videos_csv <- read_csv("data/party_videos.csv")
```
...
# selbe Informationen, nur unterschiedliche Label
```
...
# selbe Anzahl an Einträgen wie im ursprünglichen Datensatz, da jedes Video eine einzigartige ID hat und jedem Video nur eine Partei zugeordnet ist. Der Datensatz hat jetzt allerdings nur noch die beiden Variablen id und party
```
...
Überprüfe, ob es einen Unterschied zwischen `party_videos` und `party_videos_distinct` gibt. Hinweis: da `distinct()` duplizierte Reihen (in diesem Fall identische IDs) entfernt, sollte sich, falls ein Unterschied besteht, die Anzahl Reihen unterscheiden. Dies könnt ihr mit `nrow()` oder `summarise()` und `n()` prüfen. Außerdem erlaubt es `identical()`, zu prüfen, ob zwei Objekte vollkommen identisch sind. Lest dazu auch die Helpfiles von `identical()`.

```{r}
party_videos %>% nrow()

party_videos_distinct %>% summarise(videos = n())

identical(party_videos, party_videos_distinct)
```
...
```{r}
party_videos %>% filter(party == "fdp")

party_videos %>% filter(contentDetails.videoPublishedAt >= ymd("1.8.2022"))

party_videos %>% filter(party == "fdp" & 
                          contentDetails.videoPublishedAt >= ymd("1.9.2021") &
                          contentDetails.videoPublishedAt <= ymd("30.9.2021"))
```
...
```{r}
party_videos %>% distinct(kind) # nur ein Wert

party_videos %>% distinct(etag) # je ein einzigartiger Wert pro Video

party_videos %>% distinct(id, contentDetails.videoId) # beide je einzigartig pro Video

party_videos %>% distinct(party, party_label) # unterschiedliche Labels, selbe Information

party_videos_2 <- party_videos %>% 
  select(contentDetails.videoId,          # eine ID ausreichend
         contentDetails.videoPublishedAt, # Publikationszeitpunkt relevant
         party_label) %>%                 # ein Parteiindikator ausreichend
  rename(video_id = contentDetails.videoId, # Vereinfachung der Variablennamen
         published_at = contentDetails.videoPublishedAt)
```
...
```{r}
party_videos_2 <- party_videos_2 %>% 
  mutate(month = floor_date(published_at, unit = "month"))
```
...
```{r}
party_videos_monthly <- party_videos_2 %>% 
...
```{r}
ggplot(party_summary, aes(x = party_label, y = total_videos)) +
  geom_col()
```
...
```{r}
ggplot(party_summary, aes(y = party_label, x = total_videos)) +
  geom_col()
```
...
```{r}
ggplot(party_summary, aes(y = party_label, x = total_videos, fill = party_label)) +
  geom_col() +
  guides(fill = "none")
```
...
ggplot(party_summary, aes(y = party_label, x = total_videos, fill = party_label)) +
  ...
  guides(fill = "none") +
  scale_fill_manual(values = colors) +
  labs(x = "Videos", y = "Partei", title = "Parteivideos gesamt") +
  theme_light()
```
...
```{r}
party_comments_january %>% summarise(comments = n(), .by = party_label)
```
...
```{r}
party_comments_january <- party_comments_january %>% 
  mutate(day = as_date(comment_PublishedAt)) 

party_comments_january %>% filter(day == dmy("31.1.2022"))

party_comments_january %>% filter(party == "cdu")

party_comments_january %>% filter(video_id == "8aqEV5Sa4d4")

party_comments_january %>% filter(day == dmy("13.1.2022") & 
                                    (party == "cdu" | party == "spd"))
...
```{r}
party_comments_january %>% summarise(comments = n(), .by = c(day, party_label)) %>% 
  ggplot(aes(x = day, y = comments, color = party_label)) +
  geom_point()
