...
output:
  ...
  powerpoint_presentation:
    fig_width: 9
    fig_height: 7
    reference_doc: "D:/academicCloud/EPINETZ/EPINetz-Team/Konferenzen/ECPR 22"
  ioslides_presentation:
    logo: "D:/academicCloud/EPINETZ/hildesheim-logo.jpg"
    fig_width: 9
    fig_height: 7
    highlight: tango
  beamer_presentation:
...
```{r, echo=FALSE, fig.cap="https://developers.google.com/static/youtube/images/yt-data-api-search_720.png?hl=de", out.height="80%", out.width="80%"}
...
-   Nachteile
    -   Ergebnisse sind auf das beschränkt, was die Endpoints der API ausgeben
...
```{r, echo=FALSE, fig.cap="", out.width="80%", out.height="80%"}
...
```{r, echo=FALSE, fig.cap="", out.width=900, out.height=700}
knitr::include_graphics("discourse.png")
```
...
## Textverarbeitung der Nutzer\*innenkommentare
...
dfm %>% textstat_frequency(groups = party_label) %>% # get top tokens for each party
  group_by(group) %>% slice_max(docfreq, n = 3, with_ties = F) %>%      # top tokens by doc frequency (occurrence in unique documents) rather than total occurrence
  select(!rank) %>%           # drop rank of total frequency (we sort by docfreq)
  knitr::kable()                    ## we can see that e.g. certain emojis are used a lot of times in the same document, boosting their frequency but not their doc frequency
...
```{r, echo = F}
kwic <- tokens %>% tokens_group(party_label) %>% kwic("deutschland")

kwic %>% group_by(docname) %>% slice_head(n = 2) %>% select(docname, pre, keyword, post) %>% knitr::kable()
```

## Ähnlichkeiten in der Wortverwendung
...
# plot as a dendrogram
plot(cluster_simi, xlab = "", sub = "", main = "Cosinusähnlichkeit der (normalisierten) Wortverwendung")

```


## Bonus I: Vorgeschlagene Videos
...
## Bonus II: Structural Topic Modeling
...
## Effekte der Partei-Variable auf Topics

```{r, echo = F, comment=""}
effects <- estimateEffect(c(3, 5) ~ party, stmobj = stm, metadata = out$meta)
